...
class Border(Window):
    ...
    def set_hyprland_margins(self):
        ...
        bottom = style.borders.bottom.height if style.borders.enabled and style.borders.bottom.enabled else 0
    ...
    def draw_background(self, widget, ctx: cairo.Context, width, height):
        style = StyleManager.get_default().style
        if style.borders.gradient is None:
            r, g, b, a = utils.hex_to_rgba(
                StyleManager.get_default().style.borders.color)
            ctx.set_source_rgba(r, g, b, a)
        else:
            options = style.borders.gradient
            gradient = cairo.LinearGradient(width*options.start.x,
                                            height*options.start.y,
                                            width*options.end.x,
                                            height*options.end.y)

            r, g, b, a = utils.hex_to_rgba(options.start.color)
            gradient.add_color_stop_rgba(0, r, g, b, a)
            r, g, b, a = utils.hex_to_rgba(options.end.color)
            gradient.add_color_stop_rgba(1, r, g, b, a)
